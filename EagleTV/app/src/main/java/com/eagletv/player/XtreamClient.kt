...
class XtreamClient {
    ...
    private val liveCache = Collections.synchronizedMap(mutableMapOf<String, List<Channel>>())
    private val categoryCache = Collections.synchronizedMap(mutableMapOf<String, List<Category>>())
    ...
    private suspend fun text(url: String) = withContext(Dispatchers.IO) {
        http.newCall(Request.Builder().url(url).header("User-Agent", "EagleTV/1.4").build()).execute().use {
            if (!it.isSuccessful) error("Server returned ${it.code}")
    ...
    suspend fun authenticate(p: Playlist): String? = runCatching {
        if (p.kind == "m3u") {
            val content = text(p.m3uUrl.ifBlank { p.server })
            return@runCatching if (content.lineSequence().firstOrNull()?.trim()?.startsWith("#EXTM3U") == true) null else "This is not a valid M3U playlist"
        }
    ...
    suspend fun categories(p: Playlist, kind: String): List<Category> {
        ...
        val result = if (p.kind == "m3u") {
            if (kind != "live") emptyList() else liveChannels(p).map { it.categoryId }.distinct().map { Category(it, it) }
        } else json(apiUrl(p, "get_${kind}_categories")).asJsonArray.mapNotNull { e ->
    ...
    suspend fun liveChannels(p: Playlist, categoryId: String? = null): List<Channel> {
        liveCache[p.id]?.let { list -> return categoryId?.let { id -> list.filter { it.categoryId == id } } ?: list }
        if (p.kind == "m3u") {
            val parsed = parseM3u(text(p.m3uUrl.ifBlank { p.server }))
            liveCache[p.id] = parsed
            return categoryId?.let { id -> parsed.filter { it.categoryId == id } } ?: parsed
        }
        val extra = categoryId?.let { "&category_id=${enc(it)}" }.orEmpty()
        val parsed = parseStreams(json(apiUrl(p, "get_live_streams", extra)).asJsonArray, "live")
        if (categoryId == null) liveCache[p.id] = parsed
        return parsed
    }

    suspend fun movies(p: Playlist, categoryId: String? = null): List<Channel> {
        if (p.kind == "m3u") return emptyList()
        val extra = categoryId?.let { "&category_id=${enc(it)}" }.orEmpty()
    ...
    fun streamUrl(p: Playlist, item: Channel): String = item.directUrl ?: when (item.streamType) {
    ...
    private fun parseM3u(content: String): List<Channel> {
        ...
        val attributes = Regex("([A-Za-z0-9_-]+)=\"([^\"]*)\"")
        var index = 0
        while (index < lines.size) {
            val info = lines[index]
            if (info.startsWith("#EXTINF", true)) {
                val attrs = attributes.findAll(info).associate { it.groupValues[1].lowercase() to it.groupValues[2] }
                val name = info.substringAfterLast(',').trim().ifBlank { attrs["tvg-name"].orEmpty().ifBlank { "Channel" } }
                var next = index + 1
                while (next < lines.size && lines[next].startsWith("#")) next++
                if (next < lines.size) {
                    val url = lines[next]
                    val group = attrs["group-title"].orEmpty().ifBlank { "Other" }
                    result += Channel(
                        id = url.hashCode() and 0x7fffffff, name = name, logo = attrs["tvg-logo"], categoryId = group,
                        epgId = attrs["tvg-id"], streamType = "live", extension = if (url.contains(".m3u8", true)) "m3u8" else "ts", directUrl = url
                    )
                    index = next
                }
            }
